...
import { Bell, Video, User, Settings, LogOut, ShieldCheck, LifeBuoy, CheckCircle2, XCircle, Trash2 } from "lucide-react";
import { useFirestore, useDoc, useMemoFirebase, useCollection, useFirebase } from "@/firebase";
import { doc, collection, query, where, limit, orderBy, writeBatch, getDocs } from "firebase/firestore";
...
export function Header() {
  ...
  const totalNotifications = (acceptedRequests?.length || 0) + unreadSystemNotifs.length;
      ...
      <div className="flex h-24 items-center justify-between px-4 md:px-8">
        <div className="flex items-center gap-4">
          ...
          <Link href="/" className="flex items-center gap-2 mr-2 group">
            <div className="bg-primary p-2 rounded-xl shadow-md transition-transform group-hover:scale-110 flex items-center justify-center w-12 h-12 overflow-hidden">
            ...
            {settings?.logoUrl ? (
              <img src={settings.logoUrl} alt="Full Logo" className="h-16 w-auto hidden sm:inline-block object-contain" />
            ) : (
              <span className="font-black text-3xl font-headline hidden sm:inline-block text-primary">{settings?.siteTitle || "فهمني"}</span>
            )}
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-4">
            ...
            <DropdownMenuContent align="end" className="w-80 p-2 rounded-2xl shadow-2xl border-2" dir="rtl">
              <div className="flex justify-between items-center p-3">
                ...
                {unreadSystemNotifs.length > 0 && (
                  <Button variant="ghost" size="sm" onClick={markAllRead} className="text-xs font-bold text-primary h-8 hover:bg-primary/5">تحديد كقروء</Button>
                )}
              </div>
                ...
                {totalNotifications > 0 ? (
                  <>
                    {systemNotifs?.map((n: any) => (
                      <DropdownMenuItem 
                        key={n.id} 
                        className={`p-4 rounded-xl cursor-pointer hover:bg-muted border-b last:border-0 ${!n.read ? 'bg-primary/5' : ''}`}
                      >
                    ...
                    {acceptedRequests?.map((req: any) => (
                      ...
                      >
                        <div className="flex gap-4">
                          ...
                          <div className="space-y-1">
                            <p className="font-black text-sm leading-tight text-right">محاضرة جاهزة: {req.title}</p>
                            <p className="text-[10px] text-muted-foreground text-right">اضغط للدخول الآن</p>
                          </div>
                        </div>
                      </DropdownMenuItem>
                    ))}
                  ...
                  <div className="p-8 text-center text-muted-foreground text-sm font-bold opacity-50 italic">
                    لا توجد تنبيهات جديدة
                  </div>
                )}
              </div>
              <DropdownMenuSeparator />
              <Button variant="ghost" onClick={() => router.push('/requests')} className="w-full font-black text-primary hover:text-primary py-2 text-sm">عرض كافة الطلبات</Button>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <DropdownMenu>
                ...
                <div className="hidden md:flex flex-col text-left items-end">
                  <div className="flex items-center gap-1">
                    ...
                    {profile?.role === 'mufhem' && profile?.isVerified && (
                      <img src={verifiedBadgeUrl} alt="Verified" className="h-3 w-3" />
                    )}
                  </div>
            ...
            <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl shadow-2xl border-2" dir="rtl">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => router.push('/profile')} className="p-3 rounded-xl cursor-pointer flex justify-end gap-3">
                <span className="font-black">الملف الشخصي</span>
                <User className="h-4 w-4 text-primary" />
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => router.push('/wallet')} className="p-3 rounded-xl cursor-pointer flex justify-end gap-3">
                <span className="font-black">المحفظة</span>
                <Settings className="h-4 w-4 text-accent" />
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={handleLogout} className="p-3 rounded-xl cursor-pointer text-destructive focus:text-destructive flex justify-end gap-3">
                <span className="font-black">خروج</span>
