  ...
  const handleCreateTicket = async () => {
    ...
    try {
      const ticketRef = await addDoc(collection(firestore, "supportTickets"), {
        userId: user.uid,
        userName: user.displayName || "مستخدم فهمت",
      ...
      await addDoc(collection(firestore, "supportTickets", ticketRef.id, "messages"), {
        senderId: user.uid,
        senderName: user.displayName || "مستخدم فهمت",
    ...
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-16 mb-20" dir="rtl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2 border-r-8 border-primary pr-6">
          <h1 className="text-4xl md:text-5xl font-black font-headline">مركز المساعدة والأسئلة الشائعة</h1>
          <p className="text-muted-foreground text-lg">تواصل مع فريق فهمت، نحن هنا لخدمتك.</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[550px] rounded-[2.5rem]" dir="rtl">
            <DialogHeader>
              ...
              <DialogDescription className="text-right font-bold">أخبرنا بمشكلتك وسيقوم فريق فهمت بحلها في أقرب وقت.</DialogDescription>
            </DialogHeader>
            <div className="py-6 space-y-6">
              ...
              <div className="space-y-2 text-right">
                ...
                <Textarea placeholder="اشرح المشكلة بالتفصيل لفريق دعم فهمت..." className="h-40 rounded-xl p-4 border-2" value={newTicket.message} onChange={(e) => setNewTicket({ ...newTicket, message: e.target.value })} />
              </div>
            </div>
            <DialogFooter>
      ...
      <Card className="rounded-[3rem] border-2 border-primary/20 bg-primary/5 overflow-hidden shadow-2xl">
        <CardHeader className="bg-primary p-8 text-white text-right">
          <div className="flex items-center justify-end gap-4">
            <div>
              <CardTitle className="text-3xl font-black">مساعد "فهمت" الذكي</CardTitle>
              <CardDescription className="text-white/80 text-lg font-bold">إجابات فورية حول سياسات وعمليات فهمت.</CardDescription>
            </div>
        ...
        <CardContent className="p-8 space-y-6">
          <div className="flex gap-4">
            <Input placeholder="مثال: كيف يتم سحب الأرباح من فهمت؟" className="h-16 rounded-2xl text-xl bg-white shadow-inner border-2 text-right" value={aiQuery} onChange={(e) => setAiQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAiAsk()} />
            <Button onClick={handleAiAsk} disabled={isAiLoading} className="h-16 px-10 rounded-2xl bg-primary text-white text-xl font-black">
              {isAiLoading ? <Sparkles className="animate-spin h-6 w-6" /> : "اسأل المساعد"}
            </Button>
          </div>
          {aiResponse && (
      ...
      <div className="space-y-8">
        <h3 className="text-3xl font-black flex items-center justify-end gap-3">تذاكر الدعم والأسئلة <History className="text-primary" /></h3>
        {selectedTicket ? (
          <Card className="shadow-2xl rounded-[2.5rem] overflow-hidden border-2 bg-white">
            <div className="p-6 bg-muted/30 border-b flex justify-between items-center">
              <div className="flex items-center gap-4">
                <Button variant="ghost" onClick={() => setSelectedTicket(null)}><ChevronRight className="ml-2" /> العودة للقائمة</Button>
                <div className="text-right">
                  <h4 className="font-black text-lg">{selectedTicket.subject}</h4>
                  <p className="text-[10px] text-muted-foreground flex items-center gap-1 justify-end"><Hash size={12}/> {selectedTicket.id.slice(0,8)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                {selectedTicket.status !== 'closed' && (
                  <Button variant="outline" size="sm" onClick={closeTicket} className="rounded-xl font-bold border-green-500 text-green-600 hover:bg-green-50">
                    <CheckCircle2 size={16} className="ml-1" /> تم حل المشكلة
                  </Button>
                )}
                ...
                {messages?.map((msg: any) => (
                  <div key={msg.id} className={`flex ${msg.isAdmin ? 'justify-start' : 'justify-end'}`}>
                    <div className={`max-w-[80%] p-4 rounded-3xl shadow-sm ${msg.isAdmin ? 'bg-white border text-zinc-800' : 'bg-primary text-white'}`}>
                      ...
                      <span className="text-[10px] opacity-50 block mt-1">{new Date(msg.createdAt).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="p-6 border-t bg-white">
              <div className="flex gap-2">
                ...
                <Button type="button" variant="ghost" size="icon" onClick={() => fileInputRef.current?.click()} className="h-14 w-14 rounded-xl text-zinc-400 hover:bg-zinc-50" disabled={selectedTicket.status === 'closed'}>
                  <Paperclip size={24} />
                </Button>
                <Input placeholder={selectedTicket.status === 'closed' ? "هذه التذكرة مغلقة" : "اكتب ردك هنا..."} className="h-14 rounded-xl border-2 pr-6" value={chatMessage} onChange={(e) => setChatMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} disabled={selectedTicket.status === 'closed'} />
                <Button onClick={() => handleSendMessage()} className="h-14 px-8 rounded-xl bg-primary shadow-lg" disabled={selectedTicket.status === 'closed'}><Send size={20}/></Button>
              </div>
            </div>
          </Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {tickets?.map((t) => (
              <Card key={t.id} className="shadow-lg border-2 rounded-[2rem] overflow-hidden bg-white hover:shadow-xl transition-all cursor-pointer group" onClick={() => setSelectedTicket(t)}>
                ...
                <CardContent className="p-8 space-y-4 text-right">
                  <h4 className="text-xl font-black text-zinc-800 group-hover:text-primary transition-colors">{t.subject}</h4>
                  <div className="flex justify-between items-center text-xs text-muted-foreground pt-4">
                    ...
                    <span className="text-primary font-black flex items-center gap-1">متابعة الدردشة <ChevronRight size={12} className="rotate-180"/></span>
                  </div>
                </CardContent>
              </Card>
            ))}
            {(!tickets || tickets.length === 0) && (
              <div className="col-span-full py-20 text-center bg-zinc-50 rounded-[3rem] border-4 border-dashed border-zinc-200">
                ...
                <p className="font-black text-2xl text-zinc-400">لا توجد تذاكر دعم مفتوحة حالياً باسمك.</p>
              </div>
            )}
          </div>
        )}
