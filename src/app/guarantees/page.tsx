    ...
    <div className="p-6 md:p-10 max-w-6xl mx-auto space-y-12 mb-20" dir="rtl">
      ...
      <div className="text-center space-y-6 relative py-10">
        ...
        <h1 className="text-4xl md:text-7xl font-black font-headline tracking-tight text-zinc-900 leading-tight">
          ضمان <span className="text-primary">الحقوق</span>
        </h1>
        <p className="text-muted-foreground text-xl md:text-2xl max-w-4xl mx-auto leading-relaxed font-bold">
          نحن في "فهمني" نلعب دور الوسيط الضامن لتجربة عادلة ومرضية، حيث تبقى حقوقك المالية والمعرفية في أمان تام.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <GuaranteeCard 
          ...
          desc="لا تبدأ أي جلسة إلا بعد تأكد المنصة من وجود رصيد لا يقل عن قيمة الجلسة في حساب المستفهم."
        />
        <GuaranteeCard 
          icon={Video} 
          title="التوثيق الرقمي للجلسة" 
          desc="كل ثانية صوتية ومرئية مسجلة ومحفوظة لدينا مؤقتاً لمراجعتها في حال حدوث أي نزاع."
        />
        <GuaranteeCard 
          icon={Target} 
          title="شرط تحقيق الهدف" 
          desc="لا يتم تحرير المبلغ للمفهم إلا بعد التأكد من تحقيق الهدف المرجو من الاستفهام."
        />
        <GuaranteeCard 
          icon={Star} 
          title="نظام تقييم شفاف" 
          desc="تقييم حقيقي للمفهمين لا يمكن حذفه أو التلاعب به لضمان المصداقية التامة."
        />
        <GuaranteeCard 
          icon={Scale} 
          title="العدالة في التقييم" 
          desc="يحق للمستخدم الاعتراض على التقييمات الكيدية، ويقوم فريقنا بحذفها إذا ثبت عدم صحتها."
        />
        <GuaranteeCard 
          icon={Microscope} 
          title="حماية الجهد العلمي" 
          desc="نضمن للمفهم حقه كاملاً في حال كان الشرح وافياً والمستفهم يحاول التلاعب."
        />
        <GuaranteeCard 
          icon={Gavel} 
          title="نظام النزاعات اليدوي" 
          desc="في حال الشكوى، تتدخل إدارة المنصة للمراجعة والفصل بين الطرفين بمهنية عالية."
        />
        <GuaranteeCard 
          icon={RefreshCcw} 
          title="حق استرداد الرصيد" 
          desc="نضمن للمستفهم استرداد مبلغه كاملاً في حال ثبت عدم تمكن المفهم من إيصال المعلومة."
        />
        <GuaranteeCard 
          icon={UserCheck} 
          title="الاعتراف الطوعي" 
          desc="نوفر خيار الاعتراف بالخطأ للمفهم لإنهاء النزاع ودياً وتخفيف العواقب الإدارية."
        />
        <GuaranteeCard 
          icon={Lock} 
          title="أمن بيانات الدفع" 
          desc="جميع عمليات الدفع داخل المنصة مؤمنة تماماً بأحدث بروتوكولات الحماية العالمية."
        />
        <GuaranteeCard 
          icon={AlertTriangle} 
          title="حق تعليق الميزات" 
          desc="يحق للموقع تجميد ميزات الحساب عند الاشتباه في مخالفة أمنية لحماية المستخدمين."
        />
        <GuaranteeCard 
          icon={Gavel} 
          title="التحكيم الملزم" 
          desc="يكون حكم الدعم الفني بناءً على تسجيل الجلسة نهائياً وملزماً للطرفين."
        />
        <GuaranteeCard 
          icon={EyeOff} 
          title="سرية التسجيلات" 
          desc="التسجيلات مرجع لفض النزاعات فقط، ولا يحق لأي طرف تسريبها أو استخدامها خارج المنصة."
        />
        <GuaranteeCard 
          icon={Ban} 
          title="منع التعاملات الخارجية" 
          desc="الالتزام بالدفع والتواصل داخل المنصة هو الشرط الوحيد لضمان حقك القانوني والمالي."
        />
        <GuaranteeCard 
          icon={Filter} 
          title="فلترة الاستفهامات" 
          desc="نضمن عدم نشر أي محتوى يخالف الشريعة أو القانون أو يتضمن استشارات حساسة."
        />
      </div>
      ...
      <Card className="rounded-[3rem] bg-zinc-900 text-white overflow-hidden shadow-2xl relative border-none">
        ...
        <CardContent className="p-10 md:p-16 space-y-8 relative z-10 text-right">
          <div className="flex items-center gap-4 justify-end">
            <h3 className="text-3xl md:text-5xl font-black font-headline">أمان التجربة والخصوصية</h3>
          ...
          <div className="pt-6 border-t border-white/10 flex items-center gap-3 justify-end text-primary font-black">
            <Info size={20} />
            <span>نظام "فهمني" الذكي يطبق هذه القواعد آلياً.</span>
          </div>
        </CardContent>
      </Card>
    </div>
