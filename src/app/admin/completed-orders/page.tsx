    ...
    <div className="p-6 md:p-10 space-y-10" dir="rtl">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-r-8 border-green-600 pr-6">
        ...
        <div className="relative w-full md:w-96">
          ...
          <Input 
            placeholder="ابحث برقم الطلب، العنوان، أو الاسم..." 
      ...
      <Card className="shadow-2xl rounded-[2.5rem] overflow-hidden border-2 bg-white">
        <Table>
          <TableHeader className="bg-green-50 h-16">
            <TableRow>
              <TableHead className="text-right px-8 font-black text-green-900">رقم الطلب</TableHead>
              ...
              <TableHead className="text-right font-black text-green-900">وسيلة الدفع</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={order.id} className="h-24 hover:bg-green-50/30 transition-colors">
                <TableCell className="px-8 font-mono text-xs font-bold text-zinc-500">
                  #{order.id.slice(-8).toUpperCase()}
                </TableCell>
                <TableCell>
                  <div className="flex flex-col text-right">
                    <span className="font-bold text-zinc-800 line-clamp-1">{order.title}</span>
                    <span className="text-[10px] text-muted-foreground">{order.mustafhemName} & {order.mufhemName}</span>
                  </div>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Badge variant="outline" className="bg-white border-zinc-200 text-zinc-600 font-bold px-3 py-1">
                    محفظة فهمت
                  </Badge>
                </TableCell>
        ...
        <DialogContent className="sm:max-w-[700px] rounded-[3rem] p-0 overflow-hidden border-none shadow-2xl" dir="rtl">
          ...
          <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto bg-[#F8FAFC]">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <DetailBox icon={FileText} label="الفاتورة" value={selectedOrder?.invoiceNumber || "N/A"} />
              <DetailBox icon={CreditCard} label="طريقة الدفع" value="محفظة المنصة (إيداع مسبق)" />
